...
public class AddonElement
{
    ...
    public AddonElement(final AddonInfo addonInfo, final Consumer<AddonInfo.AddonActionState> callbackAction) {
    ...
    public void draw(final int x, final int y, final int maxX, final int maxY, final int mouseX, final int mouseY, final boolean showSettingsButton) {
        ...
        if (this.lastActionState == AddonInfo.AddonActionState.ERROR) {
            ...
            draw.drawRect(x + iconWidth, y, x + iconWidth + progressbarWidth, maxY, ModColor.toRGB(50, 100, 50, this.isMouseOver() ? 90 : 70));
            draw.drawRect(x + iconWidth + totalWidth, y, x + iconWidth + progressbarWidth, maxY, ModColor.toRGB(50, this.isAddonInstalled() ? 100 : 50, 50, this.isMouseOver() ? 90 : 70));
        }
        else {
        ...
        final String titleString = ModColor.cl(this.isAddonInstalled() ? "a" : (verified ? "e" : "f")) + ModColor.cl("l") + ModColor.createColors(this.addonInfo.getName());
        ...
        if (verified) {
            bib.z().N().a(ModTextures.MISC_FEATURED);
        ...
        if (this.lastActionState == AddonInfo.AddonActionState.INSTALL_REVOKE) {
            ...
            if (this.drawButton(ModTextures.BUTTON_DOWNLOAD, y, 14, 2, marginX, marginY, maxX, maxY, mouseX, mouseY)) {
        ...
        if (this.lastActionState == AddonInfo.AddonActionState.INSTALL) {
            if (this.installProgress >= 100.0) {
                ...
                draw.drawRightString((int)(this.installProgress * 10.0) / 10.0 + "%", maxX - 10, y + (maxY - y - 8) / 2);
            }
        }
    ...
    private boolean drawButton(final nf resourceLocation, final int y, final int buttonSize, final int buttonPadding, int marginX, final int marginY, final int maxX, final int maxY, final int mouseX, final int mouseY) {
        final DrawUtils draw = LabyMod.getInstance().getDrawUtils();
    ...
    public void drawIcon(final int x, final int y, final int width, final int height) {
        if (this.addonInfo.getImageURL() == null) {
            bib.z().N().a(ModTextures.MISC_HEAD_QUESTION);
