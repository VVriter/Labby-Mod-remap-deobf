...
public enum KeyBindMappings
{
    ...
    private static final Map<Integer, String> BY_SCAN_CODE;
    private final String configurationName;
    private final int key;
    private final int scanCode;
    
    private KeyBindMappings(final String configurationName, final int key) {
        this(configurationName, key, -1);
    }
    
    private KeyBindMappings(final String configurationName, final int key, final int scanCode) {
        this.configurationName = configurationName;
        this.key = key;
        this.scanCode = scanCode;
    }
    
    public static int getScanCode(final String name) {
        return (KeyBindMappings.BY_NAME.get(name) == null) ? -1 : KeyBindMappings.BY_NAME.get(name);
    }
    
    public static String getConfigurationName(final int scanCode) {
        return (KeyBindMappings.BY_SCAN_CODE.get(scanCode) == null) ? KeyBindMappings.UNKNOWN.getConfigurationName() : KeyBindMappings.BY_SCAN_CODE.get(scanCode);
    }
    ...
    static {
        ...
        BY_SCAN_CODE = new HashMap<Integer, String>();
        for (final KeyBindMappings value : values()) {
            KeyBindMappings.BY_NAME.put(value.configurationName, value.scanCode);
            KeyBindMappings.BY_SCAN_CODE.put(value.scanCode, value.configurationName);
        }
    }
}
